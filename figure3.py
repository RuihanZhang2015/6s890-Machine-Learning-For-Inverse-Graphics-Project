...
def retrieve_args():
    ...
    parser.add_argument(
        "--lr", "-lr", type=float, default=1e-4, help="Learning rate"
    )
    parser.add_argument(
...
def search_weights():
    ...
    loss_configurations = []
    for a in np.linspace(0.2,0.8,4):
        for b in np.linspace(0.2,0.8-a,int((0.8-a)*5)):
...
for i in [1,5,9]:
    ...
    for dim in range(3):
        ...
        for folder in folders:
            ...
            if 'loss' not in folder:
                continue

            try:
                with open(args.out + '/' + folder + '/basin_dim_{}.pkl'.format(dim),'rb') as f:
                    result = pickle.load(f) 
            except:
                continue
            
            weight = folder.split('_')
            # print(result['right']-result['left'])
            # [[3,6]] #float(folder[-14:-10]),float(folder[-9:-5]),float(folder[-4:])
            temp.append( [float(weight[3]),float(weight[6]),result['right']-result['left']])


        plt.figure(figsize = (10,5)) 
        temp = np.asarray(temp)
        basins = temp[:,2]
        stepsize = max((max(basins)-min(basins))/20,0.0001)
        x = np.linspace(0,1,21)
        colors = mpl.colormaps['Reds'](x)

        # colors = {k:v for k,v in zip(x,colors)}
        
        for a,b,c in temp:
            # print(colors[ int( (c-min(basins))/stepsize ) ])
            plt.plot(a,b,'.',markersize = 20, c =colors[ int( (c-min(basins))/stepsize ) ],label = '{} {}'.format(a,b))
        plt.xlabel('weight for color loss')
        plt.ylabel('weight for ssim loss')
        # plt.legend()
        plt.title('dim{}'.format(dim))
        plt.show()
        # plt.colorbar()
        plt.savefig('plots/object_{}_dim{}.jpg'.format(i,dim))
