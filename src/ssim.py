...
def ms_ssim(
    ...
    for _ in range(levels):
        ...
        if torch.isnan(ssim_val).any():
            print("nan in ssim_val", ssim_val)
        cs_vals_lst.append(cs)
        ssim_vals.append(ssim_val)
        padding = (X.shape[2] % 2, X.shape[3] % 2)
        X = F.avg_pool2d(X, kernel_size=2, stride=2, padding=padding)
        Y = F.avg_pool2d(Y, kernel_size=2, stride=2, padding=padding)

    cs_vals = torch.stack(cs_vals_lst, dim=0)
    ...
    ms_ssim_val = torch.prod(
        (cs_vals[:-1] ** weights[:-1].unsqueeze(1)) *
...
def training_test():
    ...
    # do not output gif， it is huge，
    # output mkv first and convert to gif using ffmpeg
    video_use_gif = False
...
def performing_test_ssim():
    ...
    a = torch.randint(0, 255, size=(20, 3, 256, 256),
